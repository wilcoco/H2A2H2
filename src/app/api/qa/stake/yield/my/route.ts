  ...
  try {
    ...
    const rows = await withConn(async (c) => {
      const sql = `
        ...
        select s.id as stake_id, s.qa_root_id as root_id, s.qa_id, s.amount, s.lock_days, s.created_at, s.lock_until,
        ...
        where s.user_id = $1 and s.lock_until <= now()
        order by s.created_at desc`;
      const r = await c.query(sql, [userId]);
    ...
    const items = rows.map((r) => {
      ...
      const amountFactor = Math.sqrt(Number(r.amount || 0));
      const estimatedYield = Math.round(0.8 * amountFactor * Math.max(0, qualityScore) * 10);
      return {
        stakeId: r.stake_id,
        rootId: r.root_id,
